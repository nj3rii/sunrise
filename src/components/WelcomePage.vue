<template>
  <v-container class="welcome-background">
      ...
      <v-col cols="10" sm="8" md="6" lg="4">
        <v-card class="welcome-card" elevation="10">
          ...
          <v-card-title class="text-center teal darken-2 white--text py-4">
            <h1 class="text-h3 font-weight-bold w-100">
              WELCOME TO Sunrise-Community
            </h1>
          </v-card-title>
          ...
          <v-card-text v-if="isAuthenticated" class="text-center py-8">
            ...
            <div
              class="text-uppercase text-h6 font-weight-medium grey--text text--darken-1 mb-2"
            >
              {{ userEmail }}
            </div>

            <div
              class="text-uppercase text-body-1 teal--text text--darken-2 font-weight-medium"
            >
              {{ userRole }}
            </div>
            ...
            <p class="text-body-1 text-center mx-auto" style="max-width: 80%">
              Thank you for joining Sunrise . We're excited to have you on board!
...
const { isAuthenticated, currentUser, can, initialize } = useAuth();
